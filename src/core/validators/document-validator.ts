    ...
    switch (block.type) {
      case 'heading': {
        ...
        if (lastHeadingLevel > 0 && level > lastHeadingLevel + 1) {
          errors.push(
            `Heading hierarchy error: H${lastHeadingLevel} followed by H${level} (skipped H${lastHeadingLevel + 1})`
          );
        }
          ...
          if (table.rows && Array.isArray(table.rows)) {
            for (let rowIdx = 0; rowIdx < table.rows.length; rowIdx++) {
              ...
              if (Array.isArray(row) && row.length !== headerCount) {
                errors.push(
                  `Table at position ${i}, row ${rowIdx + 1}: Expected ${headerCount} columns, found ${row.length}`
                );
              }
            }
          }
 ...
 */
export function validateHeadingHierarchy(content: ContentBlock[]): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function validateTables(content: ContentBlock[]): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
    ...
    if (block.type === 'table' && typeof block.content === 'object' && 'headers' in block.content) {
      ...
      if (table.rows && Array.isArray(table.rows)) {
        for (let i = 0; i < table.rows.length; i++) {
          ...
          if (Array.isArray(row) && row.length !== headerCount) {
            errors.push(
              `Table ${tableCount}, Row ${i + 1}: Column count mismatch (expected ${headerCount}, found ${row.length})`
            );
          }
        }
      }
...
/**
 * Validate document and throw ValidationError on first error
 */
export function validateDocumentStrict(markdown: string): {
  metadata: DocumentMetadata | null;
  warnings: string[];
} {
  ...
  if (!result.valid && result.errors.length > 0) {
    ...
    // Determine validation rule from error message
    let rule = 'structure';
    let location: string | null = null;

    if (firstError.includes('Required field')) {
      rule = 'required-field';
    } else if (firstError.includes('Heading hierarchy')) {
      rule = 'heading-hierarchy';
      const match = firstError.match(/H(\d+) followed by H(\d+)/);
      if (match) {
        location = `heading level ${match[1]} to ${match[2]}`;
      }
    } else if (firstError.includes('Table')) {
      rule = 'table-structure';
      const posMatch = firstError.match(/position (\d+)/);
      const rowMatch = firstError.match(/row (\d+)/);
      if (posMatch && rowMatch) {
        location = `table ${posMatch[1]}, row ${rowMatch[1]}`;
      } else if (posMatch) {
        location = `table at position ${posMatch[1]}`;
      }
    } else if (firstError.includes('Empty heading')) {
      rule = 'empty-heading';
      const match = firstError.match(/position (\d+)/);
      if (match) {
        location = `position ${match[1]}`;
      }
    } else if (firstError.includes('Invalid')) {
      rule = 'invalid-value';
      const fieldMatch = firstError.match(/"([^"]+)"/);
      if (fieldMatch) {
        location = `field "${fieldMatch[1]}"`;
      }
    }

    throw new ValidationError(firstError, rule, location);
  }
