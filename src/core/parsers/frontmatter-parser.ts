...
export function validateFrontMatter(metadata: any): ValidationResult {
  ...
  if (!metadata.format) {
    ...
    errors.push(`Invalid format: "${metadata.format}". Must be one of: ${VALID_FORMATS.join(', ')}`);
  }
  ...
  if (metadata.description) {
    if (typeof metadata.description !== 'string') {
      ...
      warnings.push(`Description is ${metadata.description.length} characters (recommended max: 250)`);
    }
  }
  ...
  if (metadata.section_breaks && !VALID_SECTION_BREAKS.includes(metadata.section_breaks)) {
    errors.push(`Invalid section_breaks: "${metadata.section_breaks}". Must be one of: ${VALID_SECTION_BREAKS.join(', ')}`);
  }
  
  if (metadata.slide_breaks && !VALID_SLIDE_BREAKS.includes(metadata.slide_breaks)) {
    errors.push(`Invalid slide_breaks: "${metadata.slide_breaks}". Must be one of: ${VALID_SLIDE_BREAKS.join(', ')}`);
  }
  ...
  if (metadata.document_type && !VALID_DOCUMENT_TYPES.includes(metadata.document_type)) {
    errors.push(`Invalid document_type: "${metadata.document_type}". Must be one of: ${VALID_DOCUMENT_TYPES.join(', ')}`);
  }
 ...
 */
export function shouldGenerateFormat(metadata: DocumentMetadata | null, format: 'docx' | 'pptx' | 'xlsx'): boolean {
